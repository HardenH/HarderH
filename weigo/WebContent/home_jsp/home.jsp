...
<link href="${path}AmazeUI-2.4.2/assets/css/amazeui.css" rel="stylesheet"
	type="text/css" />
<link href="${path}AmazeUI-2.4.2/assets/css/admin.css" rel="stylesheet"
	type="text/css" />
...
<script type="text/javascript">
	 ...
	 $(function(){
			$.get("${path}Typeservlet",{"op":"cloth"},function(mydata,status){
				$.each(mydata.data,function(index,data){
					$(".word1").append("<a class='outer' href='${path}Goodsservlet?op=searchbyparentsid&PARENTSID="+data.PARENTSID+"'><span class='inner'><b class='text' name='PARENTSNAME' value='"+data.PARENTSID+"'>"+data.PARENTSNAME+"</b></span></a>");
					})
			})
			...
			$.get("${path}Typeservlet",{"op":"phone"},function(mydata,status){
				$.each(mydata.data,function(index,data){
					...
					$(".word2").append("<a class='outer' href='${path}Goodsservlet?op=searchbyparentsid&PARENTSID="+data.PARENTSID+"'><span class='inner'><b class='text' name='PARENTSNAME' value='"+data.PARENTSID+"'>"+data.PARENTSNAME+"</b></span></a>");
					}
					})
			})
			//美食分类
			$.get("${path}Typeservlet",{"op":"foods"},function(mydata,status){
				$.each(mydata.data,function(index,data){
					if(data.PARENTSNAME!=null){
					$(".word3").append("<a class='outer' href='${path}Goodsservlet?op=searchbyparentsid&PARENTSID="+data.PARENTSID+"'><span class='inner'><b class='text' name='PARENTSNAME' value='"+data.PARENTSID+"'>"+data.PARENTSNAME+"</b></span></a>");
					}
					})
			}) 
		})
		...
		$.get("${path}Typeservlet",{"op":"smallcloth","parentsid":data.PARENTSID},function(mydata,status){
							$.each(mydata.data,function(index,data){
								$(".PARENTSID"+data.PARENTSID).append("<a title='"+data.TYPENAME+"' href='${path}Goodsservlet?op=searchbychildid&typeid="+data.TYPEID+"'><span>"+data.TYPENAME+"</span></a>");
						})
					});
				})
				})
			...
			<ul class="message-r">
				...
				<div class="topMessage my-shangcheng">
					<div class="menu-hd MyShangcheng">
						<a href="#" target="_top" id="personcCenter"><i class="am-icon-user am-icon-fw"></i>个人中心</a>
					</div>
				</div>
